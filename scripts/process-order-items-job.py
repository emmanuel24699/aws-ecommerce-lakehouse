...
try:
    # --- Initializations ---
    logger.info("Starting Order Items ETL Job")
    ...
    # --- Main ETL Logic ---
    source_df = read_excel_from_s3(s3_input_path, order_items_schema)
    ...
    valid_records_df = deduplicated_df.filter(col("id").isNotNull())
    rejected_records_df = deduplicated_df.filter(col("id").isNull())
    ...
    logger.info(
        f"Merging {valid_count} valid records into Delta table at {order_items_delta_path}"
    )

    if not DeltaTable.isDeltaTable(spark, order_items_delta_path):
